    ...
    <PopupHeader title="Detalles de la Feria" subtitle="Revisa toda la información del evento" variant="info" />
        ...
        <div className="rounded-xl p-4 3xl:p-5 4xl:p-6 text-left space-y-4 border border-gray-100">
          ...
          <div className="pb-4">
            ...
            <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-600">{fair?.title ?? inscription?.feria_title ?? 'Actividad de exposición y venta de productos'}</p>
          </div>
          ...
          <div className="flex items-center gap-3 pb-4">
            ...
            <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-600">{fair?.location ?? inscription?.feria_location ?? 'Parque de San Ramón'}</p>
          </div>

          <div className="pb-4">
            <h3 className="text-gray-800 font-semibold text-base md:text-lg 3xl:text-xl 4xl:text-2xl">Fecha y hora</h3>
            <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-600">{fair?.time ?? inscription?.feria_time ?? 'Sábado 5 de Octubre · 10:00 am - 6:00 pm'}</p>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <div>
                ...
                <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-700 font-medium">{entrepreneurship?.name ?? inscription?.emprendimiento_name ?? '—'}</p>
                <p className="text-xs md:text-sm 3xl:text-base 4xl:text-lg text-gray-500">{entrepreneurship?.description ?? inscription?.emprendimiento_description ?? ''}</p>
              </div>
            </div>
          </div>
