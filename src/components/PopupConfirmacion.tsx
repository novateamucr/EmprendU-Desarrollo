...
export function ConfirmationPopup({ onClose, onConfirmar, fair = null, entrepreneurship = null }: ConfirmationPopupProps) {
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-80 z-50 p-4">
      <div className="bg-white p-6 md:p-8 3xl:p-10 4xl:p-12 rounded-xl max-w-md 3xl:max-w-lg 4xl:max-w-xl w-full shadow-lg text-center max-h-[90vh] overflow-auto dark:bg-backgroundDark dark:border dark:border-backgroundDark category-scroll">
        ...
        <div className="rounded-xl p-4 3xl:p-5 4xl:p-6 text-left space-y-4 border border-gray-100 dark:bg-cardDark dark:border-cardDark">
              <div className="pb-4">
                ...
                <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-600 dark:text-secondaryDark">{fair?.title ?? 'Actividad de exposición y venta de productos'}</p>
              </div>
          ...
          <div className="flex items-center gap-3 pb-4">
            <svg className="w-5 h-5 3xl:w-6 3xl:h-6 4xl:w-7 4xl:h-7 text-red-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
              ...
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 21s8-4.5 8-10a8 8 0 10-16 0c0 5.5 8 10 8 10z" />
                ...
                <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-600 dark:text-secondaryDark">{fair?.location ?? 'Parque de San Ramón'}</p>
          </div>

          <div className="pb-4">
            <h3 className="text-gray-800 dark:text-white font-semibold text-base md:text-lg 3xl:text-xl 4xl:text-2xl">Fecha y hora</h3>
                <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-600 dark:text-secondaryDark">{fair?.date ?? 'Sábado 5 de Octubre'}</p>
                <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-600 dark:text-secondaryDark">{fair?.time ?? '10:00 am - 6:00 pm'}</p>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <div>
                    ...
                    <p className="text-sm md:text-base 3xl:text-lg 4xl:text-xl text-gray-700 dark:text-white font-medium">{entrepreneurship?.name ?? '—'}</p>
        ...
        <div className="flex space-x-3 mt-6">
          ...
          <button
            className="flex-1 py-2 3xl:py-2.5 4xl:py-3 rounded-full bg-green-600 text-white font-medium hover:bg-green-700 transition text-sm md:text-base 3xl:text-lg 4xl:text-xl dark:bg-brandDark 0 dark:hover:bg-green-600"
                onClick={() => {
