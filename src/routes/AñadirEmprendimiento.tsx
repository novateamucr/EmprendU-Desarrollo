...
export function AñadirEmprendimiento() {
  ...
  const onSubmitProfile = async (data: any) => {
    ...
    if (!data.password || data.password.length < 6) {
      profileForm.setError('password', { type: 'manual', message: 'La contraseña debe tener al menos 6 caracteres' });
      return;
    }

    try {
      const roleId = data.role === 'emprendedor' ? 2 : data.role === 'administrador' ? 3 : 1;
        ...
        <div className="flex justify-center">
          <div className="w-full max-w-3xl mt-6">
            <div className="bg-white rounded-card shadow-soft border border-border p-6">
              <h1 className="text-2xl font-semibold text-primary mb-8">Añadir Emprendimiento</h1>
